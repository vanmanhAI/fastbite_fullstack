...
export class RecommendationEngineService {
  ...
  async generateUserFeatureVector(userId: number): Promise<{
    ...
    // Lấy hành vi trong 45 ngày gần nhất thay vì 30 ngày
    const recentPeriod = new Date();
    recentPeriod.setDate(recentPeriod.getDate() - 45);
    ...
    behaviors.forEach(behavior => {
      ...
      switch (behavior.behaviorType) {
        ...
        case BehaviorType.ADD_TO_CART:
          if (behavior.productId) {
            // Tăng cường trọng số cho hành vi thêm vào giỏ hàng và áp dụng luỹ thừa
            const cartCountBoost = Math.pow(behavior.count, 1.2);
            const weight = this.behaviorWeights.cart * timeDecay * cartCountBoost;
            productPreferences.set(
              behavior.productId, 
              (productPreferences.get(behavior.productId) || 0) + weight
            );
            
            // Cập nhật danh mục với trọng số cao hơn
            if (behavior.product?.categories) {
              behavior.product.categories.forEach(category => {
                favoriteCategories.set(
                  category.id, 
      ...
      const scoredProducts = products
        ...
        .map(product => {
          ...
          if (userVector.productPreferences.has(product.id)) {
            ...
            score += behaviorScore * 8; // Tăng từ 5 lên 8 để ưu tiên hành vi người dùng
          ...
          score += (product.rating || 3) * 0.1 + (product.likeCount || 0) * 0.15 + (Math.log(product.numReviews || 1) * 0.05);
